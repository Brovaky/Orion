...
if not Success then
	warn("Can`t Load icons:  " .. tostring(Response))
end	
...
local function GetIcon(IconName)
	...
	local data = Icons["48px"][IconName]
	if data then
		local assetId, size, offset = unpack(data)
...
local function LoadCfg(Config)
	...
	table.foreach(Data, function(a,b)
		if OrionLib.Flags[a] then
			...
			warn("Posral Config Loader - Could not find ", a ,b)
		end
	end)
end
...
function OrionLib:MakeWindow(WindowConfig)
	...
	WindowConfig.Name = WindowConfig.Name or "Posral"
	...
	WindowConfig.IntroText = WindowConfig.IntroText or "Posral
...
-- Replace premium flag checks
if TabConfig.PremiumOnly then
    TabConfig.PremiumOnly = false
end

-- Modify TabConfig object
for k,v in pairs(premiumOverride) do
    TabConfig[k] = v
end

-- Override premium validation functions
TabConfig.ValidatePremium = function() return true end
TabConfig.CheckPremium = function() return true end
TabConfig.IsPremiumUser = function() return true end

-- Hook container premium check
if Container.PremiumOnly ~= nil then
    Container.PremiumOnly = false
end

-- Override ElementFunction premium checks
for i,v in pairs(ElementFunction) do
    if type(v) == "function" then
        local oldFunc = v
        ElementFunction[i] = function(...)
            local args = {...}
            if args[1] and args[1].PremiumOnly then
                args[1].PremiumOnly = false
            end
